...
export async function GET(request: Request) {
  ...
  // Completed orders only (not cancelled/deleted)
  ...
  // Fetch order items joined through COMPLETED orders only, with product name
  // Use a two-step approach to avoid null product_id issues
  ...
  if (orderIds.length > 0) {
    const { data: orderItems } = await supabase
      .from('order_items')
      .select('product_id, quantity, unit_price, pricing_type, discount_percent, line_total, products(name)')
  ...
  // Product breakdown — skip rows with no product name
  ...
  for (const item of itemsArr) {
    // Skip if product info missing (deleted product, null join)
    const name = item.products?.name
    if (!name || !name.trim()) continue

    if (!productMap[name]) {
      productMap[name] = { name, qty: 0, normalSales: 0, discountSales: 0, freeQty: 0, totalRevenue: 0 }
    }
